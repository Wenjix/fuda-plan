...
export function QuadrantCanvas() {
  ...
  // Radial menu handler — lifted to container level, passes paneBounds
  const handleNodeContextMenu = useCallback(
    (nodeId: string, fsmState: string, x: number, y: number) => {
      // Find which pane the click came from by checking which pane contains the click point
      const paneEl = document.elementFromPoint(x, y)?.closest('[data-lane-id]') as HTMLElement | null;
      const paneBounds = paneEl
        ? {
            left: paneEl.getBoundingClientRect().left,
            top: paneEl.getBoundingClientRect().top,
            width: paneEl.getBoundingClientRect().width,
            height: paneEl.getBoundingClientRect().height,
          }
        : undefined;

      useRadialMenuStore.getState().open(
        nodeId,
        fsmState as NodeFSMState,
        x,
        y,
        paneBounds,
      );
